...
To use also Application Insights as a telemetry backend you have to provide APPLICATIONINSIGHTS_CONNECTION_STRING property. Copy the content of [.env.
example](./.env.example) 
into newly created .env and fill in the Application Insights connection string.
...
The trace should be visible on the Zipkin UI main page: [http://localhost:9411](http://localhost:9411) after clicking RUN QUERY button. You might need to 
adjust the time constraints filters.
